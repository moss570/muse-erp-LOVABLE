-- Update calculate_landed_costs function to allocate tax/freight by usage UOM quantity
...
BEGIN
  ...
  -- usage_unit_conversion = how many base units equal 1 usage unit
  -- e.g., if base is "5 gallon bag" and usage is "kg" with conversion=45, 
  -- then 1 bag = 45 kg, so 500 bags = 500 * 45 = 22500 kg
  SELECT COALESCE(SUM(
  ...
  FOR v_line IN 
    SELECT 
      ...
      -- Calculate this item's usage quantity
  ...
  LOOP
    DECLARE
      ...
      -- Proportional allocation of each cost type
    ...
    BEGIN
      ...
      -- Total landed cost = material cost + allocated costs
      v_total_landed := v_line.line_total + v_allocated_costs;
      
      -- Cost per base unit (for inventory valuation)
