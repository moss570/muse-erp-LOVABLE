...
const materialFormSchema = z.object({
  ...
  label_copy: z.string().optional().default(''),
...
export function MaterialFormDialog({
  ...
  const [documentToArchive, setDocumentToArchive] = useState<{
    index: number;
    name: string;
    isExpired: boolean;
  } | null>(null);
  ...
  const {
    ...
    queryFn: async () => {
      const {
        data,
        error
      } = await supabase.from('units_of_measure').select('*').eq('is_active', true).order('name');
  ...
  const {
    ...
    queryFn: async () => {
      const {
        data,
        error
      } = await supabase.from('dropdown_options').select('*').eq('is_active', true).order('sort_order');
      if (error) throw error;
  ...
  const {
    ...
    queryFn: async () => {
      const {
        data,
        error
      } = await supabase.from('listed_material_names').select('*, category:listed_material_categories(id, name, code)').eq('is_active', true).order('name');
      if (error) throw error;
  ...
  const {
    ...
    queryFn: async () => {
      const {
        data,
        error
      } = await supabase.from('listed_material_categories').select('*').eq('is_active', true).order('sort_order').order('name');
      if (error) throw error;
      return data as {
        id: string;
        name: string;
  ...
  const {
    ...
    queryFn: async () => {
      const {
        data,
        error
      } = await supabase.from('suppliers').select('*').eq('is_active', true).order('name');
      if (error) throw error;
  ...
  const {
    ...
    queryFn: async () => {
      const {
        data,
        error
      } = await supabase.from('suppliers').select('*').eq('is_active', true).in('supplier_type', ['manufacturer', 'manufacturer_distributor']).order('name');
      if (error) throw error;
      return data as Supplier[];
    }
  });
  ...
  const {
    ...
    queryFn: async () => {
      const {
        data,
        error
      } = await supabase.from('material_sub_categories').select('*').eq('is_active', true).order('sort_order').order('name');
      if (error) throw error;
      return data as {
        id: string;
        category: string;
  ...
  const {
    ...
    queryFn: async () => {
      const {
        data,
        error
      } = await supabase.from('gl_accounts').select('*').eq('is_active', true).order('account_code');
      if (error) throw error;
      return data;
    }
  });
  ...
  const {
    ...
    queryFn: async () => {
      const {
        data,
        error
      } = await supabase.from('material_category_gl_defaults').select('*, gl_account:gl_accounts(id, account_code, account_name)');
      if (error) throw error;
      return data;
    }
  });
  ...
  const {
    ...
    queryFn: async () => {
      ...
      const {
        data,
        error
      } = await supabase.from('material_purchase_units').select('*, units_of_measure:unit_id(*)').eq('material_id', material.id).eq('is_active', true);
      if (error) throw error;
      return data;
    },
    enabled: !!material?.id
  ...
  const {
    data: existingMaterialSuppliers
  } = useQuery({
    ...
    queryFn: async () => {
      ...
      const {
        data,
        error
      } = await supabase.from('material_suppliers').select('*').eq('material_id', material.id).eq('is_active', true);
      if (error) throw error;
      return data;
    },
    enabled: !!material?.id
  ...
  const {
    ...
    queryFn: async () => {
      const {
        data,
        error
      } = await supabase.from('document_requirements').select('*').contains('areas', ['materials']).eq('is_active', true).order('sort_order');
      if (error) throw error;
      return data;
    }
  });
  ...
  const {
    ...
    queryFn: async () => {
      ...
      const {
        data,
        error
      } = await supabase.from('material_documents').select('*').eq('material_id', material.id);
      if (error) throw error;
  ...
  const uploadUnitVariantPhotos = async (materialId: string) => {
    for (let i = 0; i < unitVariants.length; i++) {
      ...
      const {
        error: uploadError
      } = await supabase.storage.from('material-photos').upload(filePath, uv.photo_file);
      if (uploadError) {
        toast({
      ...
      const {
        data: urlData
      } = supabase.storage.from('material-photos').getPublicUrl(filePath);
      unitVariants[i] = {
        ...uv,
        photo_path: filePath,
  ...
  const uploadDefaultPhoto = async (materialId: string): Promise<{
    photo_path?: string;
    photo_url?: string;
    photo_added_at?: string;
  }> => {
    if (!defaultPhotoFile) {
      return {
        photo_path: defaultPhotoPath,
        photo_url: defaultPhotoUrl,
        photo_added_at: defaultPhotoAddedAt
      };
    }
    ...
    const {
      error: uploadError
    } = await supabase.storage.from('material-photos').upload(filePath, defaultPhotoFile);
    if (uploadError) {
      toast({
        title: 'Error uploading default photo',
        description: uploadError.message,
        variant: 'destructive'
      });
      return {};
      ...
      for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
        ...
        if (attempt > 0 && data.category) {
          const {
            data: freshCode,
            error: codeError
          } = await supabase.rpc('generate_material_code', {
        ...
        if (error) {
          const isDuplicateCodeError = error.message?.includes('materials_code_key') || error.message?.includes('duplicate key') || error.code === '23505';
        ...
        if (unitVariants.length > 0 && newMaterial) {
          const {
            error: puError
          } = await supabase.from('material_purchase_units').insert(unitVariants.map(uv => ({
        ...
        if (materialSuppliers.length > 0 && newMaterial) {
          const {
            error: msError
          } = await supabase.from('material_suppliers').insert(materialSuppliers.map(ms => ({
            material_id: newMaterial.id,
            supplier_id: ms.supplier_id,
  ...
  const updateMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({
        queryKey: ['material-purchase-units']
      });
      queryClient.invalidateQueries({
  ...
  const updateUnitVariant = (index: number, field: keyof UnitVariant, value: string | number | boolean | File | undefined) => {
    setUnitVariants(unitVariants.map((uv, i) => i === index ? {
      ...uv,
      [field]: value
    } : uv));
  };
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>, index: number) => {
    ...
    setDocuments(documents.map((doc, i) => i === index ? {
      ...doc,
      file,
      document_name: doc.document_name || file.name
    } : doc));
  };
  const addDocument = () => {
  ...
  const removeDocument = async (index: number) => {
    ...
    if (doc.file_path && doc.id) {
      const {
        error
      } = await supabase.storage.from('material-documents').remove([doc.file_path]);
      if (error) {
        toast({
  ...
  const archiveDocument = async (index: number) => {
    ...
    setDocuments(documents.map((d, i) => i === index ? {
      ...d,
      is_archived: true,
      archived_at: new Date().toISOString()
    } : d));
  ...
  const uploadDocuments = async (materialId: string) => {
    ...
    for (const doc of newDocs) {
      ...
      const {
        error: uploadError
      } = await supabase.storage.from('material-documents').upload(filePath, doc.file);
      if (uploadError) {
        toast({
      ...
      const {
        data: urlData
      } = supabase.storage.from('material-documents').getPublicUrl(filePath);
  ...
  const downloadDocument = async (doc: DocumentUpload) => {
    ...
    const {
      data,
      error
    } = await supabase.storage.from('material-documents').download(doc.file_path);
    if (error) {
      toast({
  ...
  return (
    <>
      ...
      <Form {...form}>
          ...
          <Tabs value={activeTab} onValueChange={setActiveTab}>
            <TabsList className={`grid w-full`} style={{
              ...
              <TabsContent value="basic" className="space-y-4 mt-4">
                ...
                {/* Sub-Category */}
                <FormField control={form.control} name="sub_category" render={({
                  field
                }) => {
                  ...
                  return <FormItem>
                        ...
                        <Select onValueChange={value => field.onChange(value === '__none__' ? null : value)} value={field.value || '__none__'} disabled={!selectedCategory}>
                          ...
                          <SelectContent>
                            ...
                            {filteredSubCategories.map(sc => <SelectItem key={sc.id} value={sc.name}>
                                {sc.name}
                              </SelectItem>)}
                          </SelectContent>
                        </Select>
                ...
                {/* GL Account */}
                <FormField control={form.control} name="gl_account_id" render={({
                  field
                }) => <FormItem>
                      ...
                      <Select onValueChange={value => field.onChange(value === '__none__' ? null : value)} value={field.value || '__none__'}>
                        ...
                        <SelectContent>
                          <SelectItem value="__none__">-- None --</SelectItem>
                          {glAccounts?.map(gl => <SelectItem key={gl.id} value={gl.id}>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <FormField control={form.control} name="manufacturer" render={({
                    field
                  }) => <FormItem>
                      ...
                      if (newValue) {
                        ...
                        if (selectedManufacturer && selectedManufacturer.supplier_type === 'manufacturer_distributor') {
                          ...
                          return <SelectTrigger className={cn(getSupplierFieldStyles(selectedMfr?.approval_status))}>
                          ...
                          <SelectContent>
                            <SelectItem value="__none__">None</SelectItem>
                            {manufacturers?.map(manufacturer => <SelectItem key={manufacturer.id} value={manufacturer.name} className={getSupplierItemStyles(manufacturer.approval_status)}>
                                <span className="flex items-center gap-2">
                                  {manufacturer.name}
                                  {manufacturer.supplier_type === 'manufacturer_distributor' && <span className="text-muted-foreground">(Mfr & Dist)</span>}
                                  {manufacturer.approval_status?.toLowerCase() !== 'approved' && <span className="text-xs text-muted-foreground">
                  ...
                  <FormField control={form.control} name="item_number" render={({
                    field
                  }) => <FormItem>
                        ...
                        if (manufacturerName && manufacturerName !== '__none__') {
                          ...
                          if (selectedManufacturer) {
                            setMaterialSuppliers(prev => prev.map(ms => ms.supplier_id === selectedManufacturer.id && ms.is_manufacturer ? {
                              ...ms,
                              supplier_item_number: e.target.value || undefined
                            } : ms));
                          }
                        }
                      }} />
                ...
                <FormField control={form.control} name="listed_material_id" render={({
                  field
                }) => {
                            ...
                            <Command>
                              ...
                              <CommandList>
                                ...
                                setListedMaterialPopoverOpen(false);
                                  ...
                                  {listedMaterials?.map(lm => <CommandItem key={lm.id} value={`${lm.code || ''} ${lm.name} ${lm.description || ''}`} onSelect={() => {
                                field.onChange(lm.id);
                                setListedMaterialPopoverOpen(false);
                              }}>
                                      <Check className={cn("mr-2 h-4 w-4", field.value === lm.id ? "opacity-100" : "opacity-0")} />
                ...
                <div className="grid grid-cols-2 gap-4">
                  <FormField control={form.control} name="base_unit_id" render={({
                    field
                  }) => <FormItem>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <FormField control={form.control} name="usage_unit_id" render={({
                    field
                  }) => <FormItem>
                  ...
                  <FormField control={form.control} name="usage_unit_conversion" render={({
                    field
                  }) => {
                    ...
                    return <FormItem>
                          ...
                          <FormControl>
                            <Input type="number" step="0.01" placeholder="e.g., 22.68" {...field} value={field.value ?? ''} onChange={e => field.onChange(e.target.value ? Number(e.target.value) : null)} disabled={!usageUnitId} />
                          </FormControl>
                          <FormDescription>
                            {usageUnitId && purchaseUnit && usageUnit ? `1 ${purchaseUnit.code} = ${field.value || '?'} ${usageUnit.code}` : 'Select a usage unit first'}
                          </FormDescription>
                          <FormMessage />
                        </FormItem>;
                  }} />
              ...
              <TabsContent value="specifications" className="space-y-6 mt-4">
                {(() => {
                  ...
                  if (!category) {
                    return <div className="text-center py-12 text-muted-foreground border rounded-md bg-muted/20">
                  ...
                  return <>
                      {isFood && <>
                        ...
                        field
                          ...
                          <FormField control={form.control} name="food_claims" render={({
                        field
                      }) => <FormItem>
                          ...
                          <FormField control={form.control} name="label_copy" render={({
                        field
                      }) => <FormItem>
                          ...
                          <FormField control={form.control} name="density" render={({
                        field
                          ...
                          <div className="grid grid-cols-2 gap-6">
                            <div className="space-y-4">
                              ...
                              <div className="grid grid-cols-2 gap-2">
                                <FormField control={form.control} name="receiving_temperature_min" render={({
                              field
                            }) => <FormItem>
                            ...
                            <div className="space-y-4">
                              ...
                              <div className="grid grid-cols-2 gap-2">
                                <FormField control={form.control} name="storage_temperature_min" render={({
                              field
                            }) => <FormItem>
                                      <FormLabel className="text-xs text-muted-foreground">Min</FormLabel>
                                      <FormControl>
                      ...
                      {isPackaging && <div className="space-y-4">
                          {/* Hide FDA food contact for Boxes - handled in Box-specific section */}
                          {!isBoxes && <FormField control={form.control} name="pkg_fda_food_contact" render={({
                        field
                          ...
                          <FormField control={form.control} name="pkg_material_type" render={({
                        field
                      }) => <FormItem>
                          ...
                          {!isBoxes && <FormField control={form.control} name="pkg_food_grade_suitable" render={({
                        field
                      }) => <FormItem className="flex flex-row items-start space-x-3 space-y-0">
                                  <FormControl>
                                    <Checkbox checked={field.value} onCheckedChange={field.onChange} />
                          ...
                          {!isBoxes && <FormField control={form.control} name="pkg_pcr_fda_approved" render={({
                        field
                      }) => <FormItem className="flex flex-row items-start space-x-3 space-y-0">
                                  <FormControl>
                                    <Checkbox checked={field.value} onCheckedChange={field.onChange} />
                          ...
                          {!isBoxes && <FormField control={form.control} name="pkg_heavy_metals_compliant" render={({
                        field
                      }) => <FormItem className="flex flex-row items-start space-x-3 space-y-0">
                                  <FormControl>
                                    <Checkbox checked={field.value} onCheckedChange={field.onChange} />
                          ...
                          {!isBoxes && <>
                              <FormField control={form.control} name="pkg_weight_kg" render={({
                          field
                        }) => <FormItem>
                                    <FormLabel>Weight of Packaging (KG)</FormLabel>
                                    <FormControl>
                                      <Input type="number" step="0.001" placeholder="0.000" {...field} value={field.value ?? ''} onChange={e => field.onChange(e.target.value ? Number(e.target.value) : null)} />
                                    </FormControl>
                                  </FormItem>} />

                              <div className="grid grid-cols-2 gap-4">
                              <FormField control={form.control} name="pkg_volume" render={({
                            field
                          }) => <FormItem>
                              ...
                              <FormField control={form.control} name="pkg_volume_uom_id" render={({
                            field
                          }) => <FormItem>
                                    ...
                                    <Select onValueChange={val => field.onChange(val === '__none__' ? null : val)} value={field.value || '__none__'}>
                                      ...
                                      <SelectContent>
                                        ...
                                        {units?.filter(u => u.unit_type === 'volume').map(unit => <SelectItem key={unit.id} value={unit.id}>
                                            {unit.name} ({unit.code})
                                          </SelectItem>)}
                                      </SelectContent>
                                    </Select>
                              ...
                              <FormField control={form.control} name="pkg_recyclable" render={({
                          field
                        }) => <FormItem className="flex flex-row items-start space-x-3 space-y-0">
                                    <FormControl>
                                      <Checkbox checked={field.value} onCheckedChange={field.onChange} />
                      ...
                      {isBoxes && <div className="space-y-6 mt-6">
                          ...
                          <div className="border rounded-lg p-4 bg-card">
                            ...
                            <div className="grid grid-cols-2 gap-4 mb-4">
                              <FormField control={form.control} name="box_strength_type" render={({
                            field
                          }) => <FormItem>
                              ...
                              <FormField control={form.control} name="box_strength_value" render={({
                            field
                          }) => <FormItem>
                            ...
                            <FormField control={form.control} name="box_flute_type" render={({
                          field
                            ...
                            <FormField control={form.control} name="box_dimensions_internal" render={({
                          field
                        }) => <FormItem className="mb-4">
                            ...
                            <div className="grid grid-cols-2 gap-4">
                              <FormField control={form.control} name="box_joint_style" render={({
                            field
                          }) => <FormItem>
                              ...
                              <FormField control={form.control} name="box_style_code" render={({
                            field
                          }) => <FormItem>
                                    <FormLabel>Box Style Code (FEFCO)</FormLabel>
                                    <FormControl>
                                      <Input placeholder="e.g., RSC 0201" {...field} value={field.value || ''} />
                                    </FormControl>
                                    <FormDescription className="text-xs">
                          ...
                          <div className="border rounded-lg p-4 bg-card">
                            ...
                            <div className="grid grid-cols-4 gap-4 mb-4">
                              <FormField control={form.control} name="box_weight_kg" render={({
                            field
                          }) => <FormItem>
                                    <FormLabel>Box Weight (kg)</FormLabel>
                                    <FormControl>
                                      <Input type="number" step="0.01" placeholder="e.g., 0.5" {...field} value={field.value ?? ''} onChange={e => field.onChange(e.target.value ? parseFloat(e.target.value) : null)} />
                                    </FormControl>
                                    <FormDescription className="text-xs">
                                      Empty box weight
                                    </FormDescription>
                              ...
                              <FormField control={form.control} name="box_length_in" render={({
                            field
                          }) => <FormItem>
                                    <FormLabel>Length (in)</FormLabel>
                                    <FormControl>
                                      <Input type="number" step="0.125" placeholder="e.g., 16" {...field} value={field.value ?? ''} onChange={e => field.onChange(e.target.value ? parseFloat(e.target.value) : null)} />
                                    </FormControl>
                                  </FormItem>} />

                              <FormField control={form.control} name="box_width_in" render={({
                            field
                          }) => <FormItem>
                                    <FormLabel>Width (in)</FormLabel>
                                    <FormControl>
                                      <Input type="number" step="0.125" placeholder="e.g., 12" {...field} value={field.value ?? ''} onChange={e => field.onChange(e.target.value ? parseFloat(e.target.value) : null)} />
                                    </FormControl>
                                  </FormItem>} />

                              <FormField control={form.control} name="box_height_in" render={({
                            field
                          }) => <FormItem>
                                    <FormLabel>Height (in)</FormLabel>
                                    <FormControl>
                                      <Input type="number" step="0.125" placeholder="e.g., 10" {...field} value={field.value ?? ''} onChange={e => field.onChange(e.target.value ? parseFloat(e.target.value) : null)} />
                                    </FormControl>
                                  </FormItem>} />
                            </div>

                            {/* Calculated Case Cube Display */}
                            {form.watch('box_length_in') && form.watch('box_width_in') && form.watch('box_height_in') && <div className="p-3 bg-muted/50 rounded-lg">
                          ...
                          <div className="border rounded-lg p-4 bg-card">
                            ...
                            <div className="space-y-4">
                              <FormField control={form.control} name="box_recycled_content_verified" render={({
                            field
                          }) => <FormItem className="flex flex-row items-start space-x-3 space-y-0">
                                    <FormControl>
                                      <Checkbox checked={field.value} onCheckedChange={field.onChange} />
                              ...
                              <FormField control={form.control} name="box_allergen_free_adhesives" render={({
                            field
                          }) => <FormItem className="flex flex-row items-start space-x-3 space-y-0">
                                    <FormControl>
                                      <Checkbox checked={field.value} onCheckedChange={field.onChange} />
                              ...
                              <FormField control={form.control} name="box_heavy_metals_coneg" render={({
                            field
                          }) => <FormItem className="flex flex-row items-start space-x-3 space-y-0">
                                    <FormControl>
                                      <Checkbox checked={field.value} onCheckedChange={field.onChange} />
                              ...
                              <FormField control={form.control} name="box_foreign_material_control" render={({
                            field
                          }) => <FormItem className="flex flex-row items-start space-x-3 space-y-0">
                                    <FormControl>
                                      <Checkbox checked={field.value} onCheckedChange={field.onChange} />
                      ...
                      {isIndustrial && <div className="space-y-4">
                          ...
                          <FormField control={form.control} name="other_hazards" render={({
                        field
                      }) => <FormItem>
              ...
              <TabsContent value="food-safety" className="space-y-4 mt-4">
                <FormField control={form.control} name="country_of_origin" render={({
                  field
                }) => <FormItem className="max-w-xs">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <FormField control={form.control} name="fraud_vulnerability_score" render={({
                    field
                  }) => <FormItem>
                        ...
                        <Select onValueChange={val => field.onChange(val === '__none__' ? '' : val)} value={field.value || '__none__'}>
                          ...
                          <SelectContent>
                            <SelectItem value="__none__">None</SelectItem>
                            {fraudVulnerabilityOptions.map(option => <SelectItem key={option.id} value={option.value}>
                                {option.label}
                              </SelectItem>)}
                          </SelectContent>
                        </Select>
                      </FormItem>} />
                  <FormField control={form.control} name="supply_chain_complexity" render={({
                    field
                  }) => <FormItem>
                        ...
                        <Select onValueChange={val => field.onChange(val === '__none__' ? '' : val)} value={field.value || '__none__'}>
                          ...
                          <SelectContent>
                            <SelectItem value="__none__">None</SelectItem>
                            {supplyChainOptions.map(option => <SelectItem key={option.id} value={option.value}>
                                {option.label}
                              </SelectItem>)}
                          </SelectContent>
                        </Select>
                ...
                <FormField control={form.control} name="authentication_method" render={({
                  field
                }) => <FormItem>
                ...
                <div className="flex flex-wrap gap-6">
                  <FormField control={form.control} name="ca_prop65_prohibited" render={({
                    field
                  }) => <FormItem className="flex items-center gap-2">
                        <FormControl>
                          <Checkbox checked={field.value} onCheckedChange={field.onChange} />
                  ...
                  <FormField control={form.control} name="coa_required" render={({
                    field
                  }) => <FormItem className="flex items-center gap-2">
                        <FormControl>
                          <Checkbox checked={field.value} onCheckedChange={field.onChange} />
              ...
              <TabsContent value="haccp" className="space-y-6 mt-4">
                ...
                <div className="space-y-4">
                  ...
                  <FormField control={form.control} name="haccp_kill_step_applied" render={({
                    field
                  ...
                  {form.watch('haccp_kill_step_applied') === false && <FormField control={form.control} name="haccp_rte_or_kill_step" render={({
                    field
                ...
                <div className="space-y-4">
                  ...
                  <FormField control={form.control} name="haccp_new_allergen" render={({
                    field
                  }) => <FormItem className="space-y-3">
                  ...
                  {form.watch('haccp_new_allergen') === true && <FormField control={form.control} name="haccp_new_allergen_name" render={({
                    field
                  }) => <FormItem className="ml-4 border-l-2 border-muted pl-4">
                  ...
                  <FormField control={form.control} name="haccp_heavy_metal_limits" render={({
                    field
                  }) => <FormItem className="space-y-3">
                ...
                <div className="space-y-4">
                  ...
                  <FormField control={form.control} name="haccp_foreign_material_controls" render={({
                    field
                  }) => {
                    const controlOptions = [{
                      ...
                      value: 'Sieves/Screens',
                      label: 'Sieves/Screens'
                    }];
                ...
                <div className="space-y-4">
                  ...
                  <div className="border rounded-md overflow-hidden">
                    <table className="w-full">
                      ...
                      <tbody>
                        {coaLimits.map((limit, index) => <tr key={limit.id} className="border-t">
                            <td className="p-1">
                              <Input value={limit.parameter} onChange={e => {
                  ...
                  {coaLimits.length === 0 && <div className="text-center py-8 text-muted-foreground border rounded-md bg-muted/20">
              ...
              <TabsContent value="unit-variants" className="space-y-6 mt-4">
                <div className="space-y-3">
                  ...
                  {(() => {
                    ...
                    return <div className="p-4 border rounded-md bg-primary/5 border-primary/20 space-y-4">
                        <div className="flex items-center justify-between">
                          <div>
                            <div className="flex items-center gap-2">
                            ...
                            <p className="text-sm text-muted-foreground mt-1">
                              ...
                              {usageUnit && usageConversion && <span className="ml-2">
                                  → {usageConversion} {usageUnit.code}
                                </span>}
                            </p>
                            {mainItemNumber && <p className="text-xs text-muted-foreground mt-1">
                          ...
                          <div className="flex items-start gap-4">
                            <div className="flex-1 space-y-2">
                              ...
                              <div className="flex items-center gap-2">
                                ...
                                {defaultPhotoFile && <span className="text-xs text-green-600">New file selected</span>}
                              </div>
                              {/* Photo Dates */}
                              {defaultPhotoAddedAt && <div className="flex items-center gap-4 text-xs text-muted-foreground">
                                  <span>
                                    Uploaded: {new Date(defaultPhotoAddedAt).toLocaleDateString()}
                                  </span>
                                  {expiryDate && <span className={defaultPhotoStale ? 'text-amber-600 font-medium' : ''}>
                                      Expires: {expiryDate.toLocaleDateString()}
                                    </span>}
                                </div>}
                            </div>
                            {defaultPhotoUrl && <div className="flex items-center gap-2">
                                <img src={defaultPhotoUrl} alt="Default product" className="h-16 w-16 rounded object-cover border" />
                                {defaultPhotoStale && <div className="flex items-center gap-1 text-amber-600">
                  ...
                  {unitVariants.length === 0 ? <div className="text-center py-6 text-muted-foreground border rounded-md bg-muted/20">
                      ...
                      return <div key={index} className="p-4 border rounded-md bg-card space-y-3">
                              ...
                              <div className="flex-1 space-y-3">
                                ...
                                <div className="flex items-start gap-4 pt-2 border-t">
                                  <div className="flex-1 space-y-2">
                                    ...
                                    {/* Photo Dates */}
                                    {uv.photo_added_at && <div className="flex items-center gap-4 text-xs text-muted-foreground">
                                        <span>
                                  ...
                                  {uv.photo_url && <div className="flex items-center gap-2">
                                      <img src={uv.photo_url} alt="Product" className="h-12 w-12 rounded object-cover border" />
                                      {photoStale && <div className="flex items-center gap-1 text-amber-600">
                                          <AlertTriangle className="h-4 w-4" />
                                          <span className="text-xs">Review needed<br />(10+ months)</span>
                                        </div>}
                                    </div>}
              ...
              <TabsContent value="suppliers" className="space-y-6 mt-4">
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {(() => {
                        ...
                        const selectedManufacturer = manufacturers?.find(m => m.name === manufacturerName);
                        if (selectedManufacturer && selectedManufacturer.supplier_type === 'manufacturer_distributor') {
                          return <Button type="button" variant="outline" size="sm" onClick={() => autoPopulateSuppliersForManufacturer(selectedManufacturer.id, form.getValues('item_number'))}>
                  ...
                  {materialSuppliers.length === 0 ? <div className="text-center py-8 text-muted-foreground border rounded-md bg-muted/20">
                      ...
                      const unitOptions = [{
                        id: '__default__',
                        label: `Default (${baseUnit?.code || 'Base Unit'})`
                      ...
                      return <div key={index} className="p-4 border rounded-md bg-card space-y-4">
                            <div className="flex items-start gap-3">
                              <div className="flex-1 space-y-4">
                                ...
                                <div className="grid grid-cols-3 gap-3">
                                    ...
                                    return <Select value={ms.supplier_id} onValueChange={value => {
                                      ...
                                      if (newSupplier && shouldShowSupplierWarning(newSupplier.approval_status)) {
                                        ...
                                        setSupplierWarningOpen(true);
                                            ...
                                            <SelectContent>
                                              {suppliers?.map(supplier => <SelectItem key={supplier.id} value={supplier.id} className={getSupplierItemStyles(supplier.approval_status)}>
                                                  <span className="flex items-center gap-2">
                                                    {supplier.name} ({supplier.code})
                                                    {supplier.approval_status?.toLowerCase() !== 'approved' && <span className="text-xs text-muted-foreground">
                                      ...
                                      {!ms.is_primary && materialSuppliers.length > 1 && <Button type="button" variant="ghost" size="sm" onClick={() => updateMaterialSupplier(index, 'is_primary', true)} className="shrink-0">
                                          Set Primary
                                        </Button>}
                                    </div>
                                  </div>
                                  <div>
                                    ...
                                    <Select value={ms.purchase_unit_id || '__default__'} onValueChange={value => updateMaterialSupplier(index, 'purchase_unit_id', value === '__default__' ? undefined : value)}>
                                      ...
                                      <SelectContent>
                                        {unitOptions.map(opt => <SelectItem key={opt.id} value={opt.id}>
                                            {opt.label}
                                          </SelectItem>)}
                                      </SelectContent>
                                    </Select>
                                ...
                                <div className="grid grid-cols-3 gap-3">
                                  <div>
                                    ...
                                    <Input value={ms.supplier_item_number || ''} onChange={e => updateMaterialSupplier(index, 'supplier_item_number', e.target.value)} placeholder="Vendor's item #" />
                                  </div>
                                  <div>
                                    <label className="text-xs font-medium text-muted-foreground mb-1 block">
                                      Cost Per Purchase Unit ($) *
                                    </label>
                                    <Input type="number" step="0.01" value={ms.cost_per_unit ?? ''} onChange={e => updateMaterialSupplier(index, 'cost_per_unit', e.target.value ? parseFloat(e.target.value) : undefined)} placeholder="0.00" />
                                  </div>
                                  <div>
                                    <label className="text-xs font-medium text-muted-foreground mb-1 block">
                                      Cost Per Usage Unit ($)
                                    ...
                                    <div className="flex items-center h-10 px-3 rounded-md border bg-muted">
                                      <span className="text-sm font-medium">
                                        {calculatedUsageCost !== null ? `$${calculatedUsageCost.toFixed(4)}` : '—'}
                                      </span>
                                      {usageUnit && <span className="text-xs text-muted-foreground ml-1">
                                          /{usageUnit.code}
                                        </span>}
                                    </div>
                                  </div>
                                ...
                                <div className="grid grid-cols-3 gap-3">
                                  <div>
                                    ...
                                    <Input type="number" value={ms.lead_time_days ?? ''} onChange={e => updateMaterialSupplier(index, 'lead_time_days', e.target.value ? parseInt(e.target.value) : undefined)} placeholder="7" />
                                  </div>
                                  <div>
                                    <label className="text-xs font-medium text-muted-foreground mb-1 block">
                                      Min Order Qty
                                    </label>
                                    <Input type="number" step="0.01" value={ms.min_order_quantity ?? ''} onChange={e => updateMaterialSupplier(index, 'min_order_quantity', e.target.value ? parseFloat(e.target.value) : undefined)} placeholder="1" />
                                  </div>
                                  <div>
                                    <label className="text-xs font-medium text-muted-foreground mb-1 block">
                                      Notes
                                    </label>
                                    <Input value={ms.notes || ''} onChange={e => updateMaterialSupplier(index, 'notes', e.target.value)} placeholder="Special instructions..." />
                                  </div>
                                </div>
                              </div>
                              <Button type="button" variant="ghost" size="icon" className="h-8 w-8 text-destructive shrink-0" onClick={() => removeMaterialSupplier(index)}>
                                <Trash2 className="h-4 w-4" />
                              </Button>
                            </div>
                            {selectedSupplier && <p className="text-xs text-muted-foreground border-t pt-2">
              ...
              <TabsContent value="documents" className="space-y-6 mt-4">
                ...
                <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                      <Button type="button" variant="ghost" size="sm" onClick={() => setShowArchivedDocs(!showArchivedDocs)} className="text-muted-foreground">
                  ...
                  {!material && <div className="p-4 border rounded-md bg-amber-500/10 border-amber-500/30">
                      <p className="text-sm text-amber-700 dark:text-amber-400">
                        💡 Save the material first to upload documents.
                      </p>
                    </div>}

                  {documents.filter(d => showArchivedDocs || !d.is_archived).length === 0 ? <div className="text-center py-8 text-muted-foreground border rounded-md bg-muted/20">
                      ...
                      return <div key={index} className={`p-4 border rounded-md bg-card space-y-4 ${doc.is_archived ? 'opacity-60 bg-muted/30' : ''}`}>
                            {doc.is_archived && <div className="flex items-center gap-2 text-sm text-muted-foreground pb-2 border-b">
                            ...
                            <div className="flex items-start justify-between gap-3">
                              <div className="flex-1 space-y-4">
                                <div className="grid grid-cols-2 gap-3">
                                  <div>
                                    ...
                                    <Input value={doc.document_name} onChange={e => updateDocument(index, 'document_name', e.target.value)} placeholder="e.g., Product Specification" disabled={!doc.isNew || doc.is_archived} className={!doc.isNew ? 'bg-muted cursor-not-allowed' : ''} />
                                  </div>
                                  <div>
                                    <label className="text-xs font-medium text-muted-foreground mb-1 block">
                                      Document Type
                                    ...
                                    <Select value={doc.requirement_id || '__none__'} onValueChange={value => updateDocument(index, 'requirement_id', value === '__none__' ? undefined : value)} disabled={!doc.isNew || doc.is_archived}>
                                      ...
                                      <SelectContent>
                                        ...
                                        {documentRequirements?.map(req => <SelectItem key={req.id} value={req.id}>
                                ...
                                <div className="grid grid-cols-2 gap-3">
                                  <div>
                                    ...
                                    {doc.file_path && !doc.file ? <div className="flex items-center gap-2">
                                        ...
                                        <Input type="file" accept=".pdf,.doc,.docx,.xls,.xlsx,.jpg,.jpeg,.png" onChange={e => handleFileUpload(e, index)} disabled={!material || !doc.isNew || doc.is_archived} className={`flex-1 ${!doc.isNew ? 'bg-muted cursor-not-allowed' : ''}`} />
                                      </div>}
                                    {doc.file && <p className="text-xs text-green-600 mt-1">
                              ...
                              <div className="flex flex-col gap-2">
                                {!doc.isNew && !doc.is_archived && <Button type="button" variant="outline" size="sm" onClick={() => handleArchiveClick(index, doc.document_name, doc.expiry_date)}>
                                    <Archive className="h-4 w-4 mr-1" />
                                    Archive
                                  </Button>}
                                {doc.is_archived && <Button type="button" variant="outline" size="sm" onClick={() => restoreDocument(index)}>
                                    <ArchiveRestore className="h-4 w-4 mr-1" />
                                    Restore
                                  </Button>}
                                {doc.isNew && <Button type="button" variant="ghost" size="icon" className="h-8 w-8 text-destructive shrink-0" onClick={() => removeDocument(index)} title="Remove">
                                    <Trash2 className="h-4 w-4" />
                                  </Button>}
                              </div>
                            </div>
                            {requirement && <p className="text-xs text-muted-foreground">
                  ...
                  {documentRequirements && documentRequirements.filter(r => r.is_required).length > 0 && <div className="pt-4 border-t">
                      ...
                      <div className="space-y-1">
                        ...
                        return <div key={req.id} className="flex items-center gap-2 text-sm">
                              {hasDoc ? <span className="text-green-600">✓</span> : <span className="text-muted-foreground">○</span>}
        ...
        <CreateUnitDialog open={createUnitOpen} onOpenChange={setCreateUnitOpen} onUnitCreated={unitId => {
